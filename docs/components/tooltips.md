...
halfway between adjacent points, a cursor line marks the selected x, each
series shows an active dot, and the tooltip follows the pointer. Bars join by
their footprint: a grouped bar chart lists every series of the category under
the pointer, with one cursor on the category centre — and no active dots, since
the bar is already the mark. `mode="y"` does the same along
the y axis — useful for horizontal layouts, but available on any Cartesian
chart. The default, `mode="nearest"`, keeps the 12 px nearest-point behavior.
