...
There is no pie *mark*, but there is a pie *chart*: `xy.pie_chart(labels,
values)` builds one slice per label, sized by value. It is a composition rather
than a new geometry — each slice is an `xy.bar()` wedge inside
`xy.polar_bar_chart()` — which keeps sector styling and export behavior on the
same renderer as radial bars. Compose the wedges yourself, as the examples above
do, when you want control the factory does not expose.
