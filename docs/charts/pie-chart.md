...
nothing to move the view, there is nothing to restore either: double-click reset
does nothing, and the reset controls drop out of the modebar by default (an
explicit `reset_axes` still grants them). Add `xy.interaction_config(zoom=True)`
to restore the wheel and double-click reset; these examples also hide the modebar
with `xy.modebar(show=False)`, so drop that child too if you want the visible zoom
and reset buttons. See
