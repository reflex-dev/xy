...
| `fig, ax = plt.subplots()`; `plt.subplots(n, m, figsize=, dpi=, squeeze=, sharex=, sharey=)` | Grid renders as gapless CSS-grid HTML sized to the figure and stitched PNG/SVG; shared axes use common domains and live linked pan/zoom. Panels divide the figure like Matplotlib's (no minimum panel size), small panels scale their margins with the cell and drop the zoom controls, and grids past the WebGL context budget (12 panels) embed native-raster tiles instead of live charts so every panel paints. `Figure.subplots_adjust(...)` is rejected because those frame/spacing values are not representable by this renderer; use `GridSpec(..., wspace=, hspace=)` for supported explicit layout |
