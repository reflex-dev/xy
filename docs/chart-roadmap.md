...
| 9 | Candlestick / OHLC | candlestick, OHLC bars, volume overlay, range selector | Implemented | `Figure().candlestick(...)` + `Figure().ohlc(...)`: body+wick / tick-bar instanced rects, up/down color, hollow candles + wick color, low..high autorange, OHLC-bucket decimation **with zoom re-decimation**, crosshair+nearest-candle snap, CPU OHLC hover. App-side (not chart): range-selector buttons, navigator, indicator computation. Chart follow-ups: rangebreak/ordinal-x gap collapse, log axis, linked cross-chart views (for app-composed volume/indicator panes). |
