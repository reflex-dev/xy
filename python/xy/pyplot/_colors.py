...
# Matplotlib's qualitative (ListedColormap) palettes, hex-sampled from
# matplotlib 3.11. They have no engine table — ``cmap="tab10"`` on a plotting
# call still fails loudly — but ``plt.cm.tab10(i)`` / ``plt.get_cmap("Set2")``
# return callable colormaps so scripts that color series themselves work.
def _hex_table(spec: str) -> tuple[str, ...]:
    return tuple(f"#{value}" for value in spec.split())


QUALITATIVE_CMAPS: dict[str, tuple[str, ...]] = {
    "tab10": _hex_table("1f77b4 ff7f0e 2ca02c d62728 9467bd 8c564b e377c2 7f7f7f bcbd22 17becf"),
    "tab20": _hex_table(
    ...
    "set1": _hex_table("e41a1c 377eb8 4daf4a 984ea3 ff7f00 ffff33 a65628 f781bf 999999"),
    "set2": _hex_table("66c2a5 fc8d62 8da0cb e78ac3 a6d854 ffd92f e5c494 b3b3b3"),
    "set3": _hex_table(
    ...
    "accent": _hex_table("7fc97f beaed4 fdc086 ffff99 386cb0 f0027f bf5b17 666666"),
    "dark2": _hex_table("1b9e77 d95f02 7570b3 e7298a 66a61e e6ab02 a6761d 666666"),
    "pastel1": _hex_table("fbb4ae b3cde3 ccebc5 decbe4 fed9a6 ffffcc e5d8bd fddaec f2f2f2"),
    "pastel2": _hex_table("b3e2cd fdcdac cbd5e8 f4cae4 e6f5c9 fff2ae f1e2cc cccccc"),
}
# Matplotlib spells these with capitals; look-ups are case-insensitive.
_QUALITATIVE_DISPLAY_NAMES = {
    "set1": "Set1",
    "set2": "Set2",
    "set3": "Set3",
    "paired": "Paired",
...
def qualitative_colormap(name: object) -> "ListedColormap | None":
    """``ListedColormap`` for a Matplotlib qualitative palette name, else None.

    Accepts the ``_r`` reversal like every other colormap name.
    """
    if not isinstance(name, str):
        return None
    key = name.lower()
    reversed_ = key.endswith("_r")
    if reversed_:
        key = key[:-2]
    table = QUALITATIVE_CMAPS.get(key)
    if table is None:
        return None
    display = _QUALITATIVE_DISPLAY_NAMES.get(key, key)
    colors: tuple[str, ...] = tuple(reversed(table)) if reversed_ else table
    return ListedColormap(list(colors), name=display + ("_r" if reversed_ else ""))
