...
# Version 2 keys the sidecar by the column's byte offset in its file (see
# `_zone_cache_path`); a version-1 sidecar may hold another view's maps, so it
# is rejected and refolded once.
...
def _zone_cache_path(arr: Any) -> str | None:
    ...
    named by the column's byte offset as well as the file: sharing one path
    let a same-length column reload another column's maps (its min/max, hence
    its axis range).
    """
    bp = _ooc.backing_path(arr)
    offset = _ooc.backing_offset(arr)
    if bp is None or offset is None:
        return None
    return f"{bp}.{offset}{_ZONE_CACHE_SUFFIX}" if offset else bp + _ZONE_CACHE_SUFFIX
