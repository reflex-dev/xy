...
### Chrome gutters and the rotated y-axis title

The plot rect is the canvas minus four gutters. `padding=[top, right, bottom,
left]` sets their base — `padding=0` gives an edge-to-edge sparkline — and
`layout()` (`python/xy/_svg.py`, shared by the SVG, PNG, and PDF exporters;
mirrored by `ChartView._layout()`) then **adds** the chrome that must exist
regardless: a bottom/top gutter per visible x axis, `86 px` (+`18` with a label)
for a vertical colorbar, and `42/54 px` for right-side y axes. Absent an explicit
`padding`, the base is `[10, 14, 42, 62]`, or `[6, 8, 36, 46]` for a compact
canvas (`width < 520`).
...
left ≥ 10 px inset + half the title's line box
     ...
     + tick_label_pad (+ the outward part of tick_length)
