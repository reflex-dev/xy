...
| `ribbon` | per-end colours are **channels** (`color`/`color_target`), not style — `style.fill` gradients are rejected so the flow gradient cannot be half-overridden | — | — | is a fill | ✅ | ✅ | ✅ `stroke_width` |
