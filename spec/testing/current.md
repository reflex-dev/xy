...
| Interaction stress | `scripts/interaction_stress_smoke.py`; focused wheel/pan/zoom/pick tests | Hard Chromium CI/browser lane with retained worker JSON | `PARTIALLY IMPLEMENTED` | Core wheel, pan, hover, crosshair, box zoom, brush, pick boundaries, and standalone worker re-bin/paint/teardown have required evidence. TST-NI-012 and TST-NI-016 are complete; the full axis/action matrix remains TST-NI-011. |
...
| Axes, layout, styling, chrome, and facets | Axis/viewport, facets, CSS mark styles, export, legend-resize, rendered-label oracle, and gallery browser tests | Mixed hard evidence | `PARTIALLY IMPLEMENTED` | Formatter semantics and focused named/log/category labels are hard-gated; the full named/reversed/log/category interaction combinations, collision catalog, and cross-renderer parity remain. See TST-NI-010 and TST-NI-011. |
