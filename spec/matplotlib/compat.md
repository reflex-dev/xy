...
| `Axes.inset_axes([left, bottom, width, height], sharex=, sharey=)` | Bounds are relative to the parent axes and may extend outside it for marginal plots. Insets are independent free-form panels in `Figure.axes`, preserve draw order, and statically share the requested axis property store; they are not flattened into the parent data space. |
