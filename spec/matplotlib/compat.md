...
| `legend()` | `loc`, columns, title/font size/colors, frame styling, `borderpad`, `labelspacing`, `borderaxespad`, `fancybox`, `framealpha`, and `shadow` are retained across browser and static output. `loc='best'` reproduces `Legend._find_best_position`: candidates in Matplotlib's location-code order, scored by vertex occupancy of the **measured** legend box inset by `borderaxespad`, first empty one wins, ties to the earlier candidate. Measured parity on the shape set in `tests/pyplot/test_best_legend_placement.py` is 7/10; the residual is dominated by xy's plot rect being larger than Matplotlib's axes rect, and substituting Matplotlib's rect lifts it to 9/10. Departures (text/patch extents counted as anchor points, no segment-crossing term, 4096-vertex decimation) are enumerated in `spec/api/styling.md` § Legend placement. An unresolved or misspelled `loc` raises instead of silently centering |
