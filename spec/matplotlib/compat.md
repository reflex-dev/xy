...
| matplotlib | notes |
|---|---|
| `plt.plot` / `ax.plot` | format strings (`'r--o'`), multiple series per call, implicit x, `label=`, `lw=`, `ls=`, `alpha=`, marker face/edge styling, directional `^`/`v`/`<`/`>` triangles and distinct `+`/`x` glyphs, `markevery`, and dependency-free affine *data* transforms (`Affine2D + ax.transData`). Legend handles retain the line's dash/gapcolor and centered marker state even when masked or NaN data materializes as independent segments; axes/figure-fraction transforms on data artists, partial fill styles, and cap/join policies fail loudly |
| `scatter(x, y, s=, c=, cmap=, vmin=, vmax=, alpha=, marker=, edgecolors=, plotnonfinite=)` | `s` (pt², area) maps to pixel diameter; numeric 1-D `c` remains a colormap encoding, while `N×3`/`N×4` face and edge colors, alpha arrays, sizes, and linewidth arrays stay in one collection. Explicit alpha replaces intrinsic RGBA alpha, matching Matplotlib. In addition to the named marker table, bounded regular `(sides, style[, angle])` tuples, finite custom vertex paths (up to 64 vertices), and single-glyph mathtext markers render distinctly in browser, SVG, PNG, and legends. Longer/multi-glyph authored paths and custom norms fail loudly |
| `bar`, `barh`, `grouped_bar`, `bar_label` | string categories, stacking bases, per-bar face/edge color-alpha pairs and linewidths, plus iterable/indexable `BarContainer.patches` views whose setters mutate the parent batched trace. Legend patch handles retain independent face, edge, and edge-width paint, including unfilled outlined bars |
...
| `fill_between(x, y1, y2, where=, step=)` / `fill_betweenx` | Masks are split into finite contiguous polygons; step geometry is expanded exactly. Unspecified fill colors consume an independent patch cycle, so preceding line plots do not change the first fill from `C0` |
| `stackplot` | All four baselines are computed by the native stacked-bounds kernel |
| `imshow` / `pcolormesh` (`cmap=`, `vmin=`/`vmax=`, `origin=`) | `imshow` defaults to `rcParams['image.origin']`; nearest stays cell-exact, while named smoothing modes use dependency-free per-kernel approximations over a bounded 512–1024 px intermediate for both scalar and RGB(A) data. Images own independent logical-source and render arrays, and `AxesImage.set_data()` rebuilds both through the same normalization, origin, and resampling path as initial creation. Filter choice and intermediate size do not yet depend on final display resolution, and explicit `interpolation="auto"` remains unsupported. Unsupported stages/transforms fail loudly. Uniform meshes retain the texture fast path; nonuniform and curvilinear grids use native quad-to-triangle expansion. Both hug their outer cell edge with no margin, as Matplotlib's sticky image/mesh edges do |
...
| `fig.add_subplot(2, 2, 1)` / `add_subplot(221)` | `Figure.add_subplot` creates a new axes even when the figure already contains the same spec; stateful `plt.subplot` instead activates the first matching axes in figure order. A no-argument `plt.axes()` creates a fresh default-position axes. Same-position axes share one static-export frame and stack in figure order, so the later opaque axes patch covers the older plot without offsetting duplicate tick/label chrome |
| `plt.subplot_mosaic([['A','B'],['C','C']])` / `Figure.subplot_mosaic` | Row sequences (a list of equal-length label strings, or nested label lists) resolve to a uniform grid; each distinct label, in first-appearance order, binds to the next cell, returning claimed `(fig, {label: Axes})` entries that a later `Figure.add_subplot` does not hijack, with `figsize=`/`dpi=` sizing the figure. Repeated labels do not span and `'.'` does not blank a cell — the grid keeps one axes per cell — and Matplotlib's single-string forms (`'AB;CC'`, newline-separated blocks) are not parsed into rows |
...
| `add_subplot(spec, sharex=, sharey=, xticklabels=[], ...)` / reused `plt.subplot(..., sharex=, sharey=)` | per-axes sharing aliases the axis-property store (static domains, as `twiny` does), not Matplotlib's live Grouper; `get_shared_x_axes()` reflects it. Construction-only sharing keywords are handled before ordinary `Axes.set(...)` properties on both creation and activation paths |
...
- Authored Matplotlib marker paths outside the bounded scatter contract,
  arbitrary clipping graphs, and unsupported collection gradients are rejected
