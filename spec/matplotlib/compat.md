...
| `plt.subplot_mosaic([['A','B'],['C','C']])` / `Figure.subplot_mosaic` | Equal-length row sequences and Matplotlib's single-string forms (`'AB;CC'` or newline-separated blocks) resolve through GridSpec geometry. Repeated labels form one rectangular spanning axes, the configurable empty sentinel leaves a real hole, width/height ratios are honored, and `sharex=`/`sharey=` link every materialized axes without creating phantom panels. Non-rectangular repeated-label regions fail loudly |
