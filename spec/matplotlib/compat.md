...
| `fig.add_subplot(2, 2, 1)` / `add_subplot(221)` | `Figure.add_subplot` creates a new axes even when the figure already contains the same spec; stateful `plt.subplot` instead activates the first matching axes in figure order. A no-argument `plt.axes()` creates a fresh default-position axes |
