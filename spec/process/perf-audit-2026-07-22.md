...
name lets a same-length column reload another column's maps. First build pays
the scan and writes the sidecar (21 MB for both planet columns); every later
build loads it bit-identically. Measured on the real 10.74 B-row planet
columns: **52.8 s →
