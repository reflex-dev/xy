...
When you edit `.github/workflows/ci.yml`, `.github/workflows/codspeed.yml`, or
benchmark artifact wiring, run:

```bash
make check-ci
```

Four release workflows are generated by `reflex-release`, which owns their
invariants — regenerate with `reflex-release sync` after editing
`[tool.reflex-release]`, and its `sync --check` runs on every pull request. This
repository owns two of its own, both wired in through that table:

- `build_release_artifacts.yml` (`custom-build`) — the wheel/wasm/sdist matrix.
  After touching it, run it manually from the Actions tab as a dry run, and add
  any new platform to `expect-artifacts` in the same change.
- `deploy-docs-stg.yml` (`post-release-workflow`) — dispatched per published tag.
  Keep its `tag`/`package`/`version` inputs: they are the dispatch contract, and
  dropping one fails the release *after* the upload.

Both contracts are in `spec/process/production-readiness.md` §§ Upgrading the
release tool, After a release.
...
`CHANGELOG.md` is generated, and it is the release trigger — a version heading
with no matching git tag is what publishes that version. Never edit a version
heading by hand (the pull-request workflow rejects new ones). Instead, every change under `python/`, `src/` or `js/` needs one, and the
`skip-changelog` label waives it when a change is genuinely not user-facing
(`root-source-dirs` in `[tool.reflex-release]` is the authority on the scope):

```bash
make news NAME=1234.feature.md   # breaking, deprecation, feature, bugfix,
                                 # performance, docs, misc
make news-check                  # the same check the pull request runs
```

A fragment is one or two sentences written for someone reading release notes,
not for a reviewer reading the diff. Use `+something.feature.md` before you know
the pull-request number and rename it later. The *Dispatch release* workflow
materializes the pending fragments into a version section; see
`spec/process/production-readiness.md` for the release pipeline itself.

## Pull Request Checklist

- A news fragment under `news/` describes every user-visible change, or the
  `skip-changelog` label says why none is needed.
