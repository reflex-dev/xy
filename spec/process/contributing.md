...
The release workflows are generated by `reflex-release`, which owns their
invariants — regenerate with `reflex-release sync` after editing
`[tool.reflex-release]`, and its `sync --check` runs on every pull request. The
one release workflow this repository owns is `build_release_artifacts.yml` (the
wheel/wasm/sdist matrix); after touching the matrix, run it manually from the
Actions tab as a dry run, and see `spec/process/production-readiness.md`
§ Upgrading the release tool for the contract it has to keep.
...
heading by hand (the pull-request workflow rejects new ones). Instead, every
change under `python/`, `src/` or `js/` adds one news fragment:
...
the pull-request number and rename it later. Apply the `skip-changelog` label
only when the change is genuinely not user-facing. The *Dispatch release*
workflow materializes the pending fragments into a version section; see
