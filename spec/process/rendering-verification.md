...
construction, *regenerable* data). Tests are excluded from the lean sdist,
but committed PNGs would still bloat every source checkout and review. Instead:
...
- The manifest check fails if a PNG appears anywhere under
  `tests/visual/`; `verify_sdist.py` independently rejects the entire
  repository-only `tests/` tree if it enters a release archive.
