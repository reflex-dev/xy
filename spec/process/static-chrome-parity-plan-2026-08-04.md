...
**Status: LANDED** (branch alek/style-compat; tests/test_chrome_parity_p2.py is the executable acceptance). Decisions taken while landing, each recorded where consumers read it:

- Flag E pinned repo-wide as recommended: pre-rotated `<polygon>` when radius==0, `<path>` with circular arcs when radius>0 (`_chromebox.rotate_points` + `_svg._rotated_box_shape` + Python-side corner rotation in `_raster._emit_slot_box`). PDF-verified both ways.
- Flag F resolved: axis_band is **navigation-gated** (badge precedent) — `capabilities.EXPORT_STATES` gained `"navigation"`, no writer emission, preflight reports state-gated. Item 7's z-order/OUT-IN-mirror work dies with the decision; the capability-matrix "clean static" row was the casualty, regenerated.
- Item 2 scope: per-instance qualifiers+geometry land in the declared snapshot for `axis_line`/`tick_mark` (the box slots), produced by `_svg.axis_chrome_boxes` — one geometry source for both writers and the snapshot. `tick_label`/`axis_title` keep single-instance records until their per-instance identity is load-bearing.
- Item 5 letter-spacing: **qualified**, not folded — the rooms already measure at the axis font size while emitters draw at slot size (the pre-existing overflow this section names); folding spacing but not size would be false precision. Recorded in the tick_label capability note.
- P0.3 repair discovered en route: `_raster._emit_slot_box` passed `(x, y, w, h)` to corner-coordinate helpers, collapsing every box off the origin; fixed with a decoded-geometry regression test. The zero-area rule (no box, no shadow, no snapshot instance) now lives in the primitives.
- P0.4's `SLOT_BOX_PROPS` did not exist in this branch yet; created in `_svg.py` per the LEGEND_BOX_PROPS pattern, consumed by writers, registry note, and preflight.
