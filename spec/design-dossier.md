...
| **Static HTML export (interactive)** | xy blob base64-embedded in the one-file artifact | 1 decode + stated 33% text expansion | base64, because HTML is a text container | size warning above threshold; offer aggregate-only embed (ship pyramid, not points). Opt-in `compress=True` makes the file self-extracting: client + blob embed as base64 of gzip (pinned mtime, deterministic) and inflate in-browser via `DecompressionStream` — roughly half the artifact at small N; needs a 2023+ browser, older ones get a plain-text notice (`spec/api/export.md` §9) |
