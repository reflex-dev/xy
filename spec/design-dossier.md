...
destroy stays direct up to 2M. An explicit per-trace `density_threshold=` (public
scatter API) replaces both auto choices and becomes that trace's visible-point budget
everywhere (`Trace.drill_budget()`). `js/src/45_lod.ts` carries the matching 200k
client default, but reads the per-trace budget the wire ships (`density.budget`,
drill-update `budget`) for its estimates.
...
| `SCATTER_DENSITY_THRESHOLD` | `200_000` | Tier-0 → Tier-2 count budget for a scatter with **no** per-point channel (`Trace.use_density()`), and the visible-count budget for view-LOD planning and drill decisions. **Default only** — a per-trace `density_threshold=` overrides it everywhere via `Trace.drill_budget()` and ships as `density.budget` / drill-update `budget` so the client tracks it. | `_trace.py`, `interaction.py`; client default `LOD_DIRECT_POINT_BUDGET` in `js/src/45_lod.ts` (`lodDrillBudget` prefers the wire value) |
...
| `DENSITY_SAMPLE_TARGET` | `8_192` | *Floor* size of the deterministic real-point sample shipped over an aggregated scatter's density texture (hybrid overlay). The per-view target ramps toward the drill budget as the visible count approaches it — `lod.density_sample_target` computes `budget²/visible`, floored here and capped at the visible count (categorical overlays cap the ramp at `budget/√k` because the stratified sqrt-share rule keeps ≈ `target·√k` rows) — so the density→points swap is continuous in drawn point count instead of an ~8k → budget cliff (LOD doc §3). Every reply records the target it used (`sample.target`). **Default only** — a per-trace `density_sample_target=` replaces the floor via `Trace.sample_target()`. | `_payload.py`, `interaction.py` (via `lod.density_sample_target`) |
