...
| `LOD_SAMPLE_FADE_COVER_HI` / `LOD_SAMPLE_FADE_COVER_LO` | `1/16` / `1/64` | Zoom-out bound on the *retained* hybrid-overlay sample (T9): drawn at full alpha while its home window covers ≥ 1/16 of the view area, hidden below 1/64, log-eased between — so a sample kept across sample-less zoom-out replies can never overplot into a false cluster. | `js/src/45_lod.js` (`lodSampleViewAlpha`) |
