...
| `LOD_SAMPLE_FADE_COVER_HI` / `LOD_SAMPLE_FADE_COVER_LO` | `1/4` / `1/32` | Fallback bound on the hybrid-overlay sample (T9): overlays ride their density windows and the drawn one is the best cached window covering the view, so the band only governs a view NO cached window covers — the best partial draws at full alpha while its window covers ≥ 1/4 of the view area, hidden below 1/32, log-eased between. The band value is a *composited* opacity target (per-point alpha solved against the expected overplot, `1−(1−band)^(1/k)`), so a partial overlay can never overplot into a false cluster. | `js/src/45_lod.ts` (`lodSampleViewAlpha`, `lodSampleForView`) |
| `LOD_SAMPLE_EXPECT_FULL` / `LOD_SAMPLE_EXPECT_NONE` | `200` / `20` | Zoom-in bound on a covering overlay (T9): fade by the expected in-view point count (`n × viewArea/winArea`) — full at ≥ 200, gone at ≤ 20, log-eased — so a handful of fixed-fraction sample dots deep inside their window never masquerades as "the data". At or above its own window an overlay is always full (a small-n exact sample is not penalized for being small). | `js/src/45_lod.ts` (`lodSampleZoomInAlpha`) |
