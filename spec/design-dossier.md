...
| `DENSITY_SAMPLE_TARGET` | `8_192` | *Floor* size of the deterministic real-point sample shipped over an aggregated scatter's density texture (hybrid overlay). The per-view target ramps toward the drill budget as the visible count approaches it — `lod.density_sample_target` computes `budget²/visible`, floored here and capped at the visible count (categorical overlays cap the ramp at `budget/√k` because the stratified sqrt-share rule keeps ≈ `target·√k` rows) — so the density→points swap is continuous in drawn point count instead of an ~8k → budget cliff (LOD doc §3). Every reply records the target it used (`sample.target`). | `_payload.py`, `interaction.py` (via `lod.density_sample_target`) |
