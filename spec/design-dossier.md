...
- **Binder builds are source builds, not wheel installs.** Binder is linux-64,
  ...
  pandas, scipy, scikit-learn, seaborn, h5py, requests) is pip-installed in
  `postBuild` from manylinux wheels rather than provisioned through conda:
  keeping ~200 packages out of the mamba solve makes the image build faster
  and sidesteps observed extraction flakes on mybinder builder nodes.
