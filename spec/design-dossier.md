...
| `LOD_SAMPLE_FADE_COVER_HI` / `LOD_SAMPLE_FADE_COVER_LO` | `1/4` / `1/32` | Zoom-out bound on the *retained* hybrid-overlay sample (T9): drawn at full alpha while its home window covers ≥ 1/4 of the view area, hidden below 1/32, log-eased between — and the band value is a *composited* opacity target (per-point alpha solved against the expected overplot, `1−(1−band)^(1/k)`), so a sample kept across sample-less zoom-out replies can never overplot into a false cluster. | `js/src/45_lod.js` (`lodSampleViewAlpha`) |
