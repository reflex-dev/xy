...
7. `dashboard_scale`: `benchmarks/bench_dashboard.py` attempts 10/20/50 mixed
   ...
   payload prep, navigation readiness, per-chart creation p50/p95/max, load and
   ...
   stable loss-free chart-count ceiling. Extended 60-chart runs pin the production
   shader inventory at 9 unique load compiles/60 links and 11/72 after the 12
   scatter pick passes; steady redraw must add neither compiles nor links. Partial
   ...
   loose catastrophic budgets to its render, scroll, and redraw timings; creation
   percentiles and long tasks remain telemetry rather than timing gates.
