...
6. **The FastAPI 100M drilldown, adapter-native** — the live drilldown scatter
   ...
   for this chart (callback endpoint, HTTP comm bridge, density-overview
   cache); here the adapter's websocket namespace and the kernel's density
   tiers do all of it, so behavioral differences between the two apps isolate
   what that custom code adds.
