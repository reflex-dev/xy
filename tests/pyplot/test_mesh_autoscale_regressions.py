...
def test_heatmap_without_coordinates_spans_its_index_cells() -> None:
    """Absent centers default to arange(n), so cells span -0.5 .. n-0.5."""
    _fig, ax = plt.subplots()
    ...
    assert ax.get_ylim() == pytest.approx((-0.5, 2.5))
...
def test_specgram_axes_are_not_pinned_to_the_unit_view() -> None:
    ...
    assert not ax._axis_is_dataless("x")
    assert not ax._axis_is_dataless("y")
    _assert_contains(_rendered(ax, "x"), float(time[0]), float(time[-1]))
    _assert_contains(_rendered(ax, "y"), float(frequency[0]), float(frequency[-1]))
