...
def test_continuous_drill_wire_is_u8_but_durable_payload_stays_raw() -> None:
    """Protocol v9 split: the live drill wire quantizes continuous
    color/size/density_val to u8 LUT coordinates (ephemeral, rebuilt per view,
    readbacks resolve kernel-side), while the durable build payload keeps the
    v7 raw encode whose domain changes are O(1) and whose buffers standalone
    hover reads back as data units (channels.ship_continuous)."""
    n = SCATTER_DENSITY_THRESHOLD + 10_000
    ...
    assert tr["color"]["enc"] == "u8" and tr["size"]["enc"] == "u8"
    ...
    # Durable build payload: same channels, raw f32, no quantization.
    ...
    for chan in (entry["color"], entry["size"]):
        assert chan["enc"] == "raw" and "dtype" not in chan
...
def test_density_view_drills_to_points_when_window_fits():
    ...
    # Live drill wire ships continuous channels as domain-baked u8 LUT
    # coordinates (enc "u8", protocol v9): the payload is rebuilt wholesale
    # per view and readbacks resolve kernel-side, so quantization is
    # invisible everywhere but the wire (§29).
    assert tr["color"]["mode"] == "continuous"
    assert tr["color"]["enc"] == "u8" and tr["color"]["dtype"] == "u8"
