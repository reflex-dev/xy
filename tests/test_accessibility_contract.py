...
# --check` proves they compile from exactly this source, so the contract
# transfers. String-literal a11y markers are re-checked in the bundles below.
CLIENTS = (
    (
        "source",
        "\n".join(
            path.read_text(encoding="utf-8") for path in sorted((ROOT / "js/src").glob("*.ts"))
        ),
    ),
)
BUNDLES = (
    ("widget bundle", (STATIC / "index.js").read_text(encoding="utf-8")),
    ("standalone bundle", (STATIC / "standalone.js").read_text(encoding="utf-8")),
)


def test_built_bundles_keep_string_level_a11y_markers() -> None:
    """The a11y layer's string payloads must ship: ARIA roles/attribute values
    and the screen-reader summary id survive minification as string literals."""
    required = ("region", "img", "status", "polite", "aria-hidden", "-summary")
    for label, text in BUNDLES:
        for snippet in required:
            assert snippet in text, f"{label} lost a11y string marker {snippet!r}"
