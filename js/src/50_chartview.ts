...
const XY_CONTEXT_GOVERNOR = {
  views: new Set(),
  seq: 1,
  // Drivers can continue counting a released context until its loss task
  // drains. Serialize deferred restore hand-offs so a burst cannot cross the
  // browser's process-wide context ceiling.
...
export class ChartView {
  ...
  _initContextLossRecovery() {
    this._listen(this.canvas, "webglcontextlost", (e) => {
      ...
      const eventCanvas = e.currentTarget as HTMLCanvasElement & {
        _xyGovernedReleasePending?: boolean;
      };
      const governedRelease =
        eventCanvas?._xyGovernedReleasePending === true ||
        eventCanvas?.dataset?.xyCtx === "released";
      // Preserve the cause for benchmark/host listeners that run after this
      // handler; recovery can change the live canvas state in the same task.
      (e as Event & { xyGovernedRelease?: boolean }).xyGovernedRelease = governedRelease;
  ...
  _releaseContext() {
    ...
    (this.canvas as HTMLCanvasElement & { _xyGovernedReleasePending?: boolean })
      ._xyGovernedReleasePending = true;
    this._ctxReleases += 1;
