...
/// average `bin_2d_mean_color` computes over raw points, following both of
/// `compose`'s regimes: downsampling area-weights each source cell across the
/// output bins it overlaps (#153 — the color plane must split exactly like
/// the counts or the banding fix would shear color off density), and
/// upsampling pulls the source cell under each output pixel. f64 accumulation
/// in a fixed traversal order — deterministic. Returns `None` for windows the
/// pyramid cannot serve and for pyramids built without color planes; the
/// caller re-bins exactly either way.
#[allow(clippy::too_many_arguments)]
