...
def _require_pr_all_paths(errors: list[str], text: str, workflow_label: str) -> None:
    """Required workflows must run on every pull request path."""
    match = re.search(r"(?ms)^  pull_request:\n(?P<body>.*?)(?=^  \w|\Z)", text)
    if match is None:
        errors.append(f"{workflow_label} workflow missing pull_request trigger")
        return
    if re.search(r"(?m)^    paths(?:-ignore)?:", match.group("body")):
        errors.append(f"{workflow_label} pull_request trigger must run for all paths")
...
def validate_ci_workflow(path: Path = DEFAULT_CI_WORKFLOW) -> list[str]:
    ...
    _require_step_contains(
        ...
        "runtime security browser smoke and evidence",
