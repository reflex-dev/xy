...
three-way harness (scripts/bench_vs.py) measures for the other libraries:
...
Usage:
  python scripts/bench_scatter_native.py [--sizes 1e4,1e5,1e6,1e7] [--render]
"""
...
from abi_smoke import load  # same dir
...
def bench_prep(lib, n: int, x: array, y: array) -> dict:  # noqa: ANN001
    ...
    if density:
        ...
        for _ in range(reps):
            ...
            lib.fc_bin_2d(_ptr(x, D), _ptr(y, D), n, -2.0, 2.0, -2.0, 2.0, GRID_W, GRID_H, _ptr(grid, F))
...
def bench_render(n: int, x: array, y: array) -> dict:  # noqa: ANN001
    ...
    if n > DENSITY_THRESHOLD:
        ...
        gmax = max(grid)
        trace = {"id": 0, "kind": "scatter", "name": "pts", "tier": "density", "n_points": n,
                 "style": {"opacity": 1.0},
                 "density": {"buf": ship_scalar(grid), "w": GRID_W, "h": GRID_H, "max": gmax,
                             "colormap": "viridis", "x_range": [-2.0, 2.0], "y_range": [-2.0, 2.0],
                             "channels_dropped": False}}
    else:
        ex = [float(v) for v in x]
        ey = [float(v) for v in y]
        trace = {"id": 0, "kind": "scatter", "name": "pts", "tier": "direct", "n_points": n,
                 "style": {"opacity": 0.7},
                 "x": ship_scalar(ex), "y": ship_scalar(ey)}
    ...
    spec = {"protocol": 2, "width": RENDER_W, "height": RENDER_H, "title": None,
            "x_axis": {"kind": "linear", "label": "x", "range": [-2.0, 2.0]},
            "y_axis": {"kind": "linear", "label": "y", "range": [-2.0, 2.0]},
            "traces": [trace], "columns": cols, "backend": "none", "show_legend": True}
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        out = subprocess.run(
            [chromium, "--headless=new", "--no-sandbox", "--disable-dev-shm-usage",
             ...
             "--virtual-time-budget=8000", "--dump-dom", p.as_uri()],
...
def main() -> None:
    ...
    for n in sizes:
        ...
        rows.append(r)
        line = (f"| {r['n']:,} | {r['tier']} | {r['data_prep_ms']:.1f} ms "
