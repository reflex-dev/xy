...
(six fresh Chromium processes total). The tables below publish fieldwise medians; derived rates,
expected intervals, and dropped intervals were recomputed from those medians. Every raw attempt
was retained and all six completed without capture errors.
...
`24d5a4ba761bb48fdbd997120a022ca3080f4bbf` on `agent/shared-webgl-spike`. Every raw file
records that revision with `runnerEnvironment.git.dirty: false`.
...
| Median productive chart presentations / second | **3,015** | **965** |
...
shared path kept every chart live through one context. Native's smaller 0.8 ms median CPU batch
p95 is not an apples-to-apples win: that batch rendered only 16 charts, while the shared batch
rendered all 50 and produced 3.12× as many useful chart presentations per second.
...
| Median observed duration | 3.0014 s | 3.0008 s |
| Productive batches | 181 | 181 |
| Expected intervals | 180 | 180 |
| Dropped intervals | 0 | 0 |
| Observed batch rate | 60.31 fps | 60.32 fps |
| Chart presentations | 9,050 | 2,896 |
| Chart presentations / second | 3,015.26 | 965.08 |
| CPU batch p50 | 1.20 ms | 0.50 ms |
| CPU batch p95 | 1.30 ms | 0.80 ms |
| CPU batch p99 | 1.40 ms | 1.00 ms |
| Shared 2D copy p95 / chart | 0.10 ms | n/a |

These are fieldwise medians of the three successful runs per profile, not a synthetic median
run. Presentation and frame rates are recomputed from the median counts and duration so the
summary remains internally consistent. The timings measure JavaScript CPU submission, not
completed GPU execution. The shared throughput and its 3.12× ratio over native are
